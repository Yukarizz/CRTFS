...
for dataset in datasets:
    ...
    ours_index = methods.index('Ours')
    methods[-1], methods[ours_index] = methods[ours_index],methods[-1]
    ...
    plt.xlabel("Threshold", fontsize=_font_size_);
    plt.xticks(fontsize=_font_size_ - 4)
    plt.ylabel("E-measure", fontsize=_font_size_);
    plt.yticks(fontsize=_font_size_ - 4)
    plt.legend(methods, loc='lower left', fontsize=_font_size_ - 2, framealpha=0.75)
    plt.savefig(os.path.join(curve_save_dir, '{}_em.png'.format(dataset)), bbox_inches='tight')