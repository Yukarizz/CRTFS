...
def Fusion_loss(ir, vi, y_f, weights=[1, 10, 10, 50], device=None):
    # grad_ir =  KF.spatial_gradient(ir, order=2).abs().sum(dim=[1,2])
    # grad_vi = KF.spatial_gradient(vi, order=2).abs().sum(dim=[1,2])
    # grad_fus = KF.spatial_gradient(fu, order=2).abs().sum(dim=[1,2])
    # grad_joint = torch.max(grad_ir, grad_vi)
    ...
    fu_grad_x, fu_grad_y = sobelconv(y_f)
    ...
    # loss_grad = F.l1_loss(grad_fus, grad_joint)
    ## SSIM损失
    loss_ssim = corr_loss(ir, vi, y_f)
    ## 强度损失
    loss_intensity = final_mse1(ir, vi, y_f) +  F.l1_loss(y_f, torch.max(ir, vi))
...
def Re_loss(ir, vi,rec_grad):
    ## img1 is the reconstructed image, img2 is the reference image
    grad_ir =  KF.sobel(ir).abs()
    grad_vi = KF.sobel(vi).abs()
    loss_grad = F.l1_loss(grad_vi, rec_grad[:,0,...].unsqueeze(1)) + F.l1_loss(grad_ir,rec_grad[:,1,...].unsqueeze(1))
    return loss_grad
