...
from kornia import color as kc
...
class SIM(nn.Module):
    ...
    def __init__(self, norm_nc, label_nc, nhidden=64):
        ...
        # self.param_free_norm = nn.LayerNorm(norm_nc, elementwise_affine=False)
    ...
    def create_window(self,window_size, channel, window_type="gaussion"):
        if window_type == "gaussion":
            _1D_window = self.gaussian(window_size, 1.5).unsqueeze(1)
            _2D_window = _1D_window.mm(_1D_window.t()).float().unsqueeze(0).unsqueeze(0)
        if window_type == "average":
            _2D_window = torch.ones(size=(1, 1, window_size, window_size))/(window_size**2)
        window = Variable(_2D_window.expand(channel, channel, window_size, window_size).contiguous())
        return window
    def calc_mean_std(self,features, eps=1e-5):
        # 计算特征图的mean和std
        '''step 3 对tensor计算窗口内的平均值'''
        mean = (F.conv2d(self.pad(features), self.window, padding=0, groups=1))
        std = torch.sqrt((F.conv2d(self.pad((features-mean)**2), self.window, padding=0, groups=1)))
    ...
    def forward(self, x, segmap):
        ...
        actv = F.interpolate(actv,[x.shape[2],x.shape[3]])
        #actv = segmap
...
class PSF(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        ########  RGB ENCODER  ########
        self.encoder_rgb_conv1 = Feature_extract(1, 64)
        ...
        self.encoder_color_layer4 = resnet_raw_model2.layer2
        self.encoder_color_layer5 = resnet_raw_model2.layer3
        self.encoder_color_layer6 = resnet_raw_model2.layer4
        ...
        self.low_fuse2_1 = SDFM(64, 32)
        # self.low_fuse2_2 = SDFM(64, 32)
        self.low_fuse1_1 = SDFM(32, 32)
        # self.low_fuse1_2 = SDFM(32, 32)
        ...
        self.SIM_color =SIM(norm_nc=32, label_nc=32, nhidden=32)
        ...
        self.pred_cbcr = SFP([self.decoder_dim_rec, 2])
    def forward(self, rgb, depth):
        ycbcr_rgb = kc.rgb_to_ycbcr(rgb)
        ...
        color4 = self.encoder_color_layer4(color3)  # (60, 80)
        ######################################################################
        rgb5 = self.encoder_rgb_layer5(rgb4)  # (30, 40)
        thermal5 = self.encoder_thermal_layer5(thermal4) # (30, 40)
        color5 = self.encoder_color_layer5(color4)

        ######################################################################
        rgb6 = self.encoder_rgb_layer6(rgb5)  # (30, 40)
        thermal6 = self.encoder_thermal_layer6(thermal5) # (30, 40)
        color6 = self.encoder_color_layer6(color5)
       ## fused featrue
       
        fused_f6 = self.high_fuse6(rgb6,thermal6,color6) # (30, 40)
        fused_f5 = self.high_fuse5(rgb5,thermal5,color5) # (30, 40)
        fused_f4 = self.high_fuse4(rgb4, thermal4,color4)# (60, 80)
        fused_f3 = self.low_fuse3_1(rgb3, thermal3)
        # fused_f3 = self.low_fuse3_2(fused_f3,color3)
        fused_f2 = self.low_fuse2_1(rgb2, thermal2)
        # fused_f2 = self.low_fuse2_2(fused_f2,color2)
        fused_f1 = self.low_fuse1_1(rgb1, thermal1)
        # fused_f1 = self.low_fuse1_2(fused_f1,color1)
        # 1/4 1/8 1/16 1/32
        ...
        binary_out, boundary_out = self.classfier(seg_f)
        ...
        rec_color = self.color_rec_decoder(self.SIM_color(color1,features_seg_rec))
        ...
        cbcr = self.pred_cbcr(rec_color)
        return binary_out, boundary_out, fused_img, vi_img, ir_img, cbcr
...
class GEFM(nn.Module):
    def __init__(self, in_C, out_C):
        ...
        self.INF_K = BBasicConv2d(out_C, out_C, 3, 1, 1)
        self.INF_V = BBasicConv2d(out_C, out_C, 3, 1, 1)
        self.Second_reduce = BBasicConv2d(in_C + out_C, out_C, 3, 1, 1)
        self.gamma1 = nn.Parameter(torch.zeros(1))
        self.gamma2 = nn.Parameter(torch.zeros(1))
        self.gamma3 = nn.Parameter(torch.zeros(1))
        self.softmax = nn.Softmax(dim=-1)
        self.color_proj = BBasicConv2d(in_C, out_C, 3, 1, 1)
        self.RGB_proj = BBasicConv2d(out_C, out_C, 3, 1, 1)
    def forward(self, x, y,color):
        Q = self.Q(color)
        RGB_K = self.RGB_K(x)
        RGB_V = self.RGB_V(x)
        m_batchsize, C, height, width = RGB_V.size()
        ...
        RGB_refine = torch.bmm(RGB_V, RGB_mask.permute(0, 2, 1))
        RGB_refine = RGB_refine.view(m_batchsize, -1, height, width)
        RGB_refine = self.gamma1 * RGB_refine + x

        RGB_refine_Q = self.RGB_proj(RGB_refine)
        INF_K = self.INF_K(y)
        INF_V = self.INF_V(y)
        INF_V = INF_V.view(m_batchsize, -1, width * height)
        INF_K = INF_K.view(m_batchsize, -1, width * height).permute(0, 2, 1)
        INF_Q = RGB_refine_Q.view(m_batchsize, -1, width * height)
        INF_mask = torch.bmm(INF_K, INF_Q)
        INF_mask = self.softmax(INF_mask)
        INF_refine = torch.bmm(INF_V, INF_mask.permute(0, 2, 1))
        INF_refine = INF_refine.view(m_batchsize, -1, height, width)
        INF_refine = self.gamma2 * INF_refine + y

        out = self.Second_reduce(torch.cat([RGB_refine, INF_refine, self.gamma3 * self.color_proj(color)], dim=1))
        return out
class PSFM(nn.Module):
    def __init__(self, in_C, out_C, cat_C):
        ...
        self.Colorobj = DenseLayer(in_C, cat_C)
        self.obj_fuse = GEFM(cat_C, out_C)
        

    def forward(self, rgb, depth, color):
        rgb_sum = self.RGBobj(rgb)        
        Inf_sum = self.Infobj(depth)
        color_sum = self.Colorobj(color)
        out = self.obj_fuse(rgb_sum,Inf_sum,color_sum)
        return out
...
class S2P2(nn.Module):
    ...
    def forward(self, feat):
        ...
        return binary_out, boundary_out
...
class cnn2token(nn.Module):
    def __init__(self,dim):
        super(cnn2token,self).__init__()
        self.map1 = nn.Linear(dim*2, dim*4)
        self.norm1 = nn.LayerNorm(dim*4)
        self.act = nn.GELU()

        self.map2 = nn.Linear(dim*4, dim)
        self.norm2 = nn.LayerNorm(dim)



    def forward(self,cnn_f,token_f):
        b, c, h, w = cnn_f.shape
        cnn_f = cnn_f.view(b, c, h * w).transpose(1, 2)

        fuse_f = torch.cat([cnn_f,token_f],dim=2)
        fuse_f = self.map1(fuse_f)
        fuse_f = self.act(self.norm1(fuse_f))


        fuse_f = self.map2(fuse_f)
        fuse_f = self.act(self.norm2(fuse_f))
        return fuse_f

class grad2token(nn.Module):
    def __init__(self,dim):
        super(grad2token,self).__init__()
        self.conv1 = nn.Conv2d(dim,dim,3,2,1)
        self.norm1 = nn.LayerNorm(dim)
        self.act = nn.GELU()
        self.conv2 = nn.Conv2d(dim,dim,3,2,1)
        self.norm2 = nn.LayerNorm(dim)

        self.map2 = nn.Linear(dim, dim*2)
        self.norm3 = nn.LayerNorm(dim*2)
    def forward(self,grad_f):
        b, c, h, w = grad_f.shape
        grad_f = self.conv1(grad_f)
        grad_f = grad_f.view(b,c,(h//2)*(w//2)).transpose(1,2)
        grad_f = self.act(self.norm1(grad_f))
        grad_f = grad_f.transpose(1,2).view(b,c,h//2,w//2)
        grad_f = self.conv2(grad_f).view(b,c,(h//4)*(w//4)).transpose(1,2)
        grad_f = self.act(self.norm2(grad_f))
        grad_f = self.map2(grad_f)
        grad_f = self.norm3(grad_f)
        return grad_f

def entropy(vi_img,ir_image):

    flat_img = vi_img.flatten()
    flat_img = flat_img*255
    counts = torch.bincount(flat_img.int())
    # Calculate probability for each pixel value
    probabilities = counts.float() / flat_img.shape[0]
    # Calculate entropy
    entropy_vi = -torch.sum(probabilities * torch.log2(probabilities.clamp(min=1e-9)))

    flat_img = ir_image.flatten()
    flat_img = flat_img*255
    counts = torch.bincount(flat_img.int())
    # Calculate probability for each pixel value
    probabilities = counts.float() / flat_img.shape[0]
    # Calculate entropy
    entropy_ir = -torch.sum(probabilities * torch.log2(probabilities.clamp(min=1e-9)))
    return 1-(torch.exp(entropy_vi)/(torch.exp(entropy_vi)+torch.exp(entropy_ir)))